...
| **Detail records**             | Tabbed drawer: usage by user, user totals, usage by interface, and the four raw event tables — each with free-text filter, click-to-sort headers, pagination, and CSV export of the filtered-and-sorted set. **Expand** takes the drawer full screen (25 rows per page instead of 10, sticky header, Escape or the backdrop to exit) for the wide tables |
