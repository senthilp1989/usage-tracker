...
- Metrics are columns, not rows: adding one touches `models.py`, an Alembic revision, `schemas.py`, `dashboard.py` (`_totals()`), and `frontend/src/types.ts` (`METRICS`). Now five: `test_cases_created/executed`, `documents_generated`, `test_cases_passed/failed`.
- "Executions" and "pass rate" (KPI tiles in `Dashboard.tsx`) are **not** stored or aggregated server-side — they're derived client-side from `test_cases_passed + test_cases_failed` at display time (`kpiExecutions`/`kpiPassRate`), separately for the server-summary and search-filtered-summary code paths. This deliberately differs from `test_cases_executed`, which counts every ingested execution row regardless of terminal status (includes QUEUED/RUNNING/STALED/CANCELLED from the source tool) — so `passed + failed` can be less than `test_cases_executed` for a still-settling day. Don't average/sum a percentage across rows; always recompute from summed passed/failed.
...
- `/dashboard/users` returns one row per (user_email, environment, report_date), not a summed total per user — it's a filtered listing of individual reports, not a rollup like `/dashboard/summary` and `/dashboard/daily` are. Filterable by `environment` and `user_email` via query params (same as `/dashboard/summary` and `/dashboard/daily`); `/dashboard/environments` lists distinct values for the frontend dropdown, same pattern as `/dashboard/user-emails`. The endpoint itself is still unpaginated (fetches the whole range); `UsersTable.tsx` paginates the already-fetched rows client-side (10/page) purely for display — CSV export still writes every row in range, not just the current page.
