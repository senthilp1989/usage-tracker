...
Standalone usage-analytics service for Test Ease. Test Ease POSTs **raw, ungrouped event rows** — one per test case creation, one per test case execution (with its terminal-or-not `status`), one per generated document — to `POST /events` on a fixed interval, batched since its own last checkpoint. No aggregation happens on ingestion: every row is a plain insert into one of three event tables, and the dashboard computes totals, pass/fail counts, and interface breakdowns by grouping those tables at query time. A custom React dashboard (which replaced an earlier Grafana setup) reads these aggregates through the API.
...
- `test_cases_passed`/`test_cases_failed` are computed via Postgres `FILTER (WHERE status = ...)` on `test_case_executed_events`, not separate queries or stored columns — this is the redundancy the old five-separate-grouped-queries NestJS design used to have.
- "Pass rate" (KPI tile in `Dashboard.tsx`) is still derived **client-side** from `test_cases_passed + test_cases_failed` at display time — this predates the rearchitecture and wasn't touched (see the fuzzy-search/pagination scoping note below).
