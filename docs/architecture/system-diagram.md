...
on but doesn't own. It POSTs **raw per-action events** (not daily totals) to
`POST /events` on a fixed interval, batched since its own checkpoint and
authenticated with a shared `X-API-Key` header; each row is a plain insert into
one of four event tables, with every total derived at query time. The
...
flowchart TD
    ...
    DB[("PostgreSQL 15<br/>4 event tables (1 row/action)")]

    TE -->|"REST POST /events, X-API-Key"| API
