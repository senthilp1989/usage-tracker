...
import { fetchExecutedEvents } from "../api";
import type { DateRange, ExecutedEventDetail } from "../types";

const PAGE_SIZE = 10;

export default function ExecutedEventsTable({
  range,
  userEmails,
  environment,
  search,
  ...
  const [items, setItems] = useState<ExecutedEventDetail[]>([]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    fetchExecutedEvents(range, page, PAGE_SIZE, userEmails, environment, search)
      .then((data) => {
        if (cancelled) return;
        setItems(data.items);
        setTotal(data.total);
      ...
      .catch(() => {
        if (!cancelled) setError("Could not load test cases executed.");
      })
    ...
    <div className={`card${loading ? " loading-dim" : ""}`}>
      <div className="card-header">
        ...
        >
          ...
          Test cases executed ({total.toLocaleString()})
        </button>
      </div>
      {expanded && (
        <>
